...
CREATE TABLE air_oai_facts.f41_traffic_t100_market_archive
	...
	, airline_old_group_nbr 		int
	, airline_new_group_nbr 		int
	, depart_airport_oai_id 		int
	, depart_airport_oai_seq_id 	int
	, depart_city_market_oai_id 	int
	...
	, depart_world_area_oai_id 		int
	, arrive_airport_oai_id 		int
	, arrive_airport_oai_seq_id 	int
	, arrive_city_market_oai_id 	int
...
*/
    ...
    ----DATA LOADING---------------
copy into air_oai_facts.f41_traffic_t100_market_archive
FROM @csv_stage
PATTERN='.*T100_MARKET_ALL.*[.]csv.gz'
...
-- refresh 
 CREATE  view air_oai_facts.airline_traffic_market_integrate_mv as
...
CREATE TABLE air_oai_facts.airline_traffic_market 
	( airline_traffic_market_key				varchar		not null
	, year_month_nbr							integer			not null
	, airline_oai_code 							varchar 		not null
	, airline_effective_date					date			not null
	, airline_entity_id							integer			not null
	, airline_entity_key						varchar		not null
	, depart_airport_oai_code 					varchar 		not null
	, depart_airport_effective_date				date			not null
	, depart_airport_history_id					integer 		not null
	, depart_airport_history_key				varchar(32)		not null
	, arrive_airport_oai_code 					varchar 		not null
	, arrive_airport_effective_date				date			not null
	, arrive_airport_history_id					integer 		not null
	, arrive_airport_history_key				varchar(32)		not null
	, service_class_code 						char(1) 		not null
	, data_source_code							varchar		not null
...
CREATE TABLE air_oai_facts.f41_traffic_t100_segment_archive
	...
	, arrive_country_name 					varchar(75)
	, arrive_world_area_oai_id 				int
	, aircraft_group_oai_nbr				int -- 
	, aircraft_type_oai_nbr 				int -- 
    ...
    ----DATA LOADING---------------
copy into air_oai_facts.f41_traffic_t100_segment_archive
FROM @csv_stage
PATTERN='.*T100_SEGMENT_ALL.*[.]csv.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
...
-- drop materialized view air_oai_facts.f41_traffic_t100_segment_load_mv;
 ...
 create view air_oai_facts.f41_traffic_t100_segment_load_mv as
...
CREATE  view air_oai_facts.airline_traffic_segment_integrate_mv as
SELECT (f.year_nbr::char(4) || lpad(f.month_nbr::varchar(2),2,'0'))::integer as year_month_nbr
     , f.service_class_code
     , f.airline_usdot_id
     , f.airline_oai_code
...
foreign key (AIRCRAFT_TYPE_OAI_NBR) references AIR_OAI.AIR_OAI_DIMS.AIRCRAFT_TYPES (AIRCRAFT_TYPE_OAI_NBR);
...
ALTER TABLE air_oai_facts.airline_traffic_segment ALTER AIRLINE_ENTITY_KEY SET DATA TYPE VARCHAR;
