...
drop schema if exists air_faa_reg;
...
create table air_faa_reg.aircraft_registry
    ...
    ----DATA LOADING---------------
copy into air_faa_reg.aircraft_registry
FROM @csv_stage
PATTERN='.*MASTER.*[.]txt.gz'
...
create table air_faa_reg.aircraft_reference
    ...
    ----DATA LOADING---------------
copy into air_faa_reg.aircraft_reference
FROM @csv_stage
PATTERN='.*ACFTREF.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
    
    -- 2 rows not loaded due to data problem
...
create table air_faa_reg.engine_reference
	...
	;
        ...
        ----DATA LOADING---------------
copy into air_faa_reg.engine_reference
FROM @csv_stage
PATTERN='.*ENGINE.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
...
create table air_faa_reg.aircraft_document_index
	...
	;
            ...
            ----DATA LOADING---------------
copy into air_faa_reg.aircraft_document_index
FROM @csv_stage
PATTERN='.*DOCINDEX.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
...
create table air_faa_reg.aircraft_reserved_registry
	...
	;
            ...
            ----DATA LOADING---------------
copy into air_faa_reg.aircraft_reserved_registry
FROM @csv_stage
PATTERN='.*RESERVED.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
...
create table air_faa_reg.aircraft_dealer_applicant
	...
	;
            ...
            ----DATA LOADING---------------
copy into air_faa_reg.aircraft_dealer_applicant
FROM @csv_stage
PATTERN='.*DEALER.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;
...
create table air_faa_reg.aircraft_deregistered
	...
	;
            ...
            ----DATA LOADING---------------
copy into air_faa_reg.aircraft_deregistered
FROM @csv_stage
PATTERN='.*DEREG.*[.]txt.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;

--1 record not loaded due to error
