...
drop  table if exists  AIR_OAI_FACTS.DB1B_AIRLINE_SURVEY_COUPON_FDW ;
...
/*server abrams_ssd8tb options 
    ...
    */
    ...
    ----DATA LOADING---------------
copy into AIR_OAI_FACTS.AIRFARE_SURVEY_TICKET_FDW  
FROM @csv_stage
PATTERN='.*Ticket.*[.]csv.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;


SELECT COUNT(*) FROM AIR_OAI_FACTS.AIRFARE_SURVEY_TICKET_FDW ;
...
/*create table air_oai_facts.airfare_survey_ticket_load
    ...
    */
CREATE TABLE air_oai_facts.airfare_survey_ticket_load CLONE air_oai_facts.airfare_survey_ticket_fdw ; -- BOTH THE TABLES ARE OF SAME STRUCTURE

SELECT COUNT(*) FROM AIR_OAI_FACTS.airfare_survey_ticket_load ;
...
INSERT INTO air_oai_facts.airfare_survey_itinerary
	( itinerary_oai_id, year_quarter_nbr, year_quarter_start_date
...
SELECT asf.itinerary_oai_id
    , asf.year_nbr||asf.quarter_nbr as year_quarter_nbr
...
select airport_oai_seq_id, count(*) from air_oai_DIMS.airport_history group by 1 having count(*) > 1 order by count(*) desc;
...
select distinct round_trip_ind from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW; -- 0/1
select distinct online_ind from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW; -- 0/1
select distinct bulk_fare_ind from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW; -- 0/1
select distinct fare_credibility_ind from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW; -- 0/1
select distinct distance_group_oai_id from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 1 to 25 int
select distinct geographic_type_oai_id from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 1/2
select distinct coupon_qty from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 1 to 12 int
select distinct passenger_qty from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 1 to 1120 int
select distinct distance_smi from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 31 to 26316 int
select distinct flown_distance_smi from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 31 to 18845 int
select distinct fare_per_person_amount_usd from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 0 to 1345400 int
select distinct fare_per_smi from air_oai_facts.AIRFARE_SURVEY_TICKET_FDW order by 1; -- 0 to 5.2093, numeric(7,5)
...
create  table air_oai_facts.db1b_airline_survey_coupon_fdw
	( itinerary_oai_id             		bigint null
...
---DATA LOADING----
copy into AIR_OAI_FACTS.DB1B_AIRLINE_SURVEY_COUPON_FDW
FROM @csv_stage
PATTERN='.*Coupon.*[.]csv.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;

SELECT COUNT(*) FROM AIR_OAI_FACTS.DB1B_AIRLINE_SURVEY_COUPON_FDW ;
...
create table air_oai_facts.airfare_survey_coupon
    ...
    , airfare_survey_coupon_pk           varchar
...
INSERT INTO air_oai_facts.airfare_survey_coupon
	(itinerary_oai_id, flight_pass_seq, year_quarter_nbr
    ,year_quarter_start_date, market_oai_id
...
SELECT ac.itinerary_oai_id
     , ac.flight_pass_seq
     , ac.year_nbr||ac.quarter_nbr as year_quarter_nbr
...
select itinerary_oai_id AS itinerary_id, flight_pass_seq, count(*) 
...
--- DATA LOADING---
copy into AIR_OAI_FACTS.AIRFARE_SURVEY_MARKET_FDW
FROM @csv_stage
PATTERN='.*Market.*[.]csv.gz'
ON_ERROR=CONTINUE FORCE = TRUE ;

----------------------------------

SELECT COUNT(*) FROM AIR_OAI_FACTS.AIRFARE_SURVEY_MARKET_FDW;
...
select itinerary_geograhic_type_oai_id, market_geograhic_type_oai_id 
     , depart_country_iso_code, depart_state_iso_code
     , arrive_country_iso_code, arrive_state_iso_code
     , count(*) 
from air_oai_facts.airfare_survey_market_fdw
group by 1,2,3,4,5,6;

select itinerary_oai_id, count(*) from air_oai_facts.airfare_survey_market_fdw group by 1 having count(*) > 1 order by count(*) desc;
select itinerary_oai_id, market_oai_id, count(*) from air_oai_facts.airfare_survey_market_fdw group by 1,2 having count(*) > 1 order by count(*) desc; -- unique
--select * from air_oai_facts.airline_survey_market_fdw where itinerary_oai_id = 202315289895;

select world_areas_group_oai_code AS wac_pair_group_oai_code, 
length(wac_pair_group_oai_code) as len_code, count(*) 
from air_oai_facts.airfare_survey_market_fdw group by 1 order by 2 desc;
...
air_oai_facts.airfare_survey_market
	( itinerary_oai_id, market_oai_id,  year_quarter_nbr    ,year_quarter_start_date
...
SELECT am.itinerary_oai_id
	 , am.market_oai_id
     ,am.year_nbr||am.quarter_nbr as year_quarter_nbr
